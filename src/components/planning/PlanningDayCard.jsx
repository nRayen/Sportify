...
const PlanningDayCard = ({date,	getSessionsForDate,openEditSessionModal, className,compact = false, editButton = false}) => {
...
>
	...
	<div className="p-3 sm:p-4">
		{getSessionsForDate(date).map((session) => (
			<div
				key={session.id}
				className="bg-primary/10 rounded-lg p-3 mb-2 last:mb-0 cursor-pointer hover:bg-primary/20 transition-colors relative"
			>
				...
				{editButton ? 
				<button className="absolute bottom-2 right-2 p-1 hover:bg-primary/30 rounded-md"
